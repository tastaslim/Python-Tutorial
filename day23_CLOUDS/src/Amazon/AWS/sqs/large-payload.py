...
class SQSClientExtended(object):
    ...
    @staticmethod
    def __string_to_base64(s):
        return base64.b64encode(s.encode('utf-8'))

    @staticmethod
    def __base64_to_string(b):
        return base64.b64decode(b).decode('utf-8')

    def set_always_through_s3(self, always_through_s3):
        """
        Sets whether all messages regardless of their payload size should be stored in Amazon S3.
        """
    ...
    def __is_base64(self, s):
        try:
            encoded = self.__string_to_base64(self.__base64_to_string(s))
            return encoded == str.encode(s)
        except Exception as e:
            print(f'Exception occurred while checking base64 format: {e}')
            return False
    ...
    def __get_msg_attributes_size(self, message_attributes):
        ...
        for key, entry in message_attributes.items():
            ...
            if entry.get('DataType'):
                total_msg_attributes_size += self.__get_string_size_in_bytes(
                    entry.get('DataType'))
            ...
            if entry.get('BinaryValue'):
                if self.__is_base64(entry.get('BinaryValue')):
                    total_msg_attributes_size += len(
                        str.encode(entry.get('BinaryValue')))
                else:
                    total_msg_attributes_size += self.__get_string_size_in_bytes(
                        entry.get('BinaryValue'))

        return total_msg_attributes_size

    def __is_large(self, message, message_attributes):
        msg_attributes_size = self.__get_msg_attributes_size(
            message_attributes)
        ...
        for message in opt_messages:
            ...
            if large_pay_load_attribute_value:
                try:
                    ...
                    if isinstance(message_body, list) and \
                            message_body[0] == "software.amazon.payloadoffloading.PayloadS3Pointer":
                    ...
                    orig_msg_body = self.get_text_from_s3(
    ...
    def __delete_message_payload_from_s3(self, receipt_handle):
        try:
            ...
            s3_object.delete()
            print(
                'Deleted s3 object https://s3.amazonaws.com/{}/{}'.format(s3_msg_bucket_name, s3_msg_key))
        except Exception as e:
    ...
    def _send_queue_message(self, queue_url, message_body, message_group_id, message_deduplication_id,
                            message_attributes, is_fifo_queue):
        """
        ...
        """
        if is_fifo_queue:
            return self.sqs.send_message(QueueUrl=queue_url, MessageBody=message_body, MessageGroupId=message_group_id,
                                         MessageDeduplicationId=message_deduplication_id,
                                         MessageAttributes=message_attributes)
        else:
            return self.sqs.send_message(QueueUrl=queue_url, MessageBody=message_body,
                                         MessageAttributes=message_attributes)

    def send_message(self, queue_url, message, message_group_id=None, message_deduplication_id=None,
        ...
        if not all([message_group_id, message_deduplication_id]):
            if any([message_group_id, message_deduplication_id]):
                raise ValueError(
                    'message_group_id and message_deduplication_id are conditionally required together')
            else:
                fifo_queue = False
        ...
        if self.always_through_s3 or self.__is_large(str(message), message_attributes):
            ...
            return self._send_queue_message(queue_url, s3_key_message, message_group_id, message_deduplication_id,
                                            message_attributes, fifo_queue)
        else:
            return self._send_queue_message(queue_url, s3_key_message, message_group_id, message_deduplication_id,
                                            message_attributes, fifo_queue)
    ...
    def __store_message_in_s3(self, message_body):
        ...
        try:
            ...
            s3.Bucket(self.s3_bucket_name).put_object(
                Key=s3_key, Body=reader.read())
            reader.close()
            opt_file.close()
