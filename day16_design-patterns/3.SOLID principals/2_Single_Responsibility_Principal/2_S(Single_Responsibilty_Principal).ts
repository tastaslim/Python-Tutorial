...
other team is responsible to display book information. They will be changing same class again and again, which might lead
to incompatible module and can cause merge conflicts.
Any issue in display feature will not affect CRUD operations too, which should not be the case.
`

console.log(
    `
     So what is the take away from this principal?
     The Class should have only one reason to change. We should not add all the logic inside single class, like CRUD operations,
     Data persistence, etc. Instead we should create another class which contains logic to store/display book information.
    `
)
