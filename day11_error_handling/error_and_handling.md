...
too, so sometimes we don't get as result what we expect. Hence, we must mke sure that we handle error and be ready for
worst case scenario.
...
In python there is finally block which is executed no matter what happened with try except.
